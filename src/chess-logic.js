...
export const isSquareUnderAttack = (row, col, color, testBoard, testColors) => {
...
export const isInCheck = (color, board, pieceColors) => {
  let kingRow, kingCol;
  for (let r = 0; r < 8; r++) {
    for (let c = 0; c < 8; c++) {
  ...
  return isSquareUnderAttack(kingRow, kingCol, color, board, pieceColors);
};
...
export const checkGameStatus = (currentPlayer, board, pieceColors, enPassantTarget) => {
  ...
  const inCheck = isInCheck(currentPlayer, board, pieceColors);
